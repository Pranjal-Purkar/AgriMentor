...
const isForward = (from: any, to: any) => {
  try { return typeof from === 'string' && typeof to === 'string' && from.endsWith('-forward') && to.endsWith('-forward'); }
  catch { return false; }
};
const isBack = (from: any, to: any) => {
  try { return typeof from === 'string' && typeof to === 'string' && from.endsWith('-back') && to.endsWith('-back'); }
  catch { return false; }
};

export const RouteAnimations = trigger('routeAnimations', [
  // FORWARD animation (use predicate)
  transition((from, to) => isForward(from, to), [
    query(':enter, :leave',
    ...
    group([
      query(':leave', [ animate('250ms ease', style({ transform: 'translateX(-10px)', opacity: 0 })) ], { optional: true }),
      query(':enter', [ animate('350ms ease', style({ transform: 'translateX(0)', opacity: 1 })) ], { optional: true })
    ])
  ]),

  // BACK animation (use predicate)
  transition((from, to) => isBack(from, to), [
    query(':enter, :leave',
      style({ position: 'absolute', left: 0, right: 0, top: 0, width: '100%' }),
      { optional: true }
    ),
